              ...
              <v-card :id="`hw-card-${hw.id}`" border :elevation="hoveredId === hw.id || clickedId === hw.id ? 12 : 2"
                ...
                <v-toolbar :color="!hw.dateTime
                  ...
                  <div class="d-flex align-center flex-grow-1 overflow-hidden" :style="{
                    ...
                    <v-icon v-if="!isLocked(hw)" size="small" class="ms-1 opacity-60">mdi-pencil-circle</v-icon>
      ...
      <v-card class="rounded-xl pa-2">
        ...
        <v-card-text>
          <v-select v-model="tempRaid" :items="['2막', '3막', '4막', '종막', '세르카']" label="레이드 선택" variant="outlined"
            color="success" class="mb-2" @update:model-value="tempDifficulty = '노말'"></v-select>
