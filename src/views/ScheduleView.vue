  ...
  <v-container fluid class="fill-height align-start pa-6">
                ...
                <div v-for="day in daysOfWeek" :key="day.fullDate" class="calendar-day-column flex-grow-1">
                  ...
                  <draggable :list="calendarSchedules[day.fullDate]" group="schedule-items" item-key="id"
                    ...
                    <template #item="{ element }">
                      ...
                      }" class="mb-1 w-100 justify-start px-1 rounded-sm schedule-chip transition-swing" label :style="isLocked(element) ? 'pointer-events: none;' : ''
                ...
                <v-toolbar :color="!bus.dateTime
                  ...
                  <div class="d-flex align-center flex-grow-1 overflow-hidden" style="min-width: 0">
                    ...
                    <div class="d-flex align-center flex-grow-1 overflow-hidden" style="min-width: 0; cursor: pointer"
                      ...
                      <v-icon size="small" class="ms-1 opacity-60" v-if="!isLocked(bus)">mdi-pencil</v-icon>
    ...
    <v-dialog v-model="editRaidDialog" max-width="400">
      <v-card class="rounded-xl pa-2">
        ...
        <v-card-text>
          <v-btn-toggle v-model="tempIsHomework" mandatory color="primary" variant="outlined" class="mb-4 w-100"
            density="compact">
          ...
          <v-select v-model="tempRaid" :items="['2막', '3막', '4막', '종막', '세르카', '지평', 'Extreme:1막', 'Extreme:2막']"
            label="레이드 선택" variant="outlined" class="mb-2" @update:model-value="tempDifficulty = '노말'"></v-select>
